...
/*
 @brief: Calculate weekday Function subprogram Weekday = {(Year+12)+Day+(Year+12)/4+T[Mtab]-1}Mod7
         If Month<3 The Year Sub 1
 @param[in]: Year,Month Offset(Mtab), Day 1990<=Year<=2099, 1<=Month<=12, 1<=Day<=31
 @return: WEEK(0=Sunday, 1=Monday, ..., 6=Saturday)
 */
#if defined(CALC_WEEK) && CALC_WEEK
void CalcWeek(struct TimeType *time)
{
    uint16_t tmp1, tmp2, tmp3;
    uint8_t weekTable[13] = { 0x00, 0x00, 0x03, 0x02, 0x05, 0x00, 0x03, 0x05, 0x01, 0x04, 0x06, 0x02, 0x04 };

    tmp1 = DecToHex(time->year);
    tmp2 = DecToHex(time->month);
    tmp3 = DecToHex(time->day);

    time->week = (tmp1 + tmp3 + (tmp1 / 4) + weekTable[tmp2] - 1) % 7;
}
#endif
